...
class LocalBrain:
    ...
    def _schema(self, tools, allow_done):
        ...
        return {
            "type": "object",
            "properties": {
                "done": {"type": "boolean", "enum": [True] if allow_done else [False]},
                "message": {"type": "string"},
    ...
    def _prompt(self, task, observation, tools, allow_done):
        ...
        completion_rule = (
            'If the task is complete, set done=true and tool="".'
            if allow_done
            else "You MUST choose a tool and set done=false."
        )
...
"""
    ...
    def think(self, task, observation, tools, allow_done=True):
        ...
        if start == -1 or end == -1 or end <= start:
            raise ValueError("Local brain returned no JSON:\n" + output[-1000:])

        try:
            data = json.loads(output[start:end + 1])
        except json.JSONDecodeError as exc:
            raise ValueError("Local brain returned invalid JSON:\n" + output[-1000:]) from exc

        if not isinstance(data, dict):
            raise ValueError("Local brain JSON is not an object:\n" + output[-1000:])

        data["done"] = bool(data.get("done", False))
        data["message"] = str(data.get("message", ""))

        tool = data.get("tool", "")
        data["tool"] = tool.strip().lower() if isinstance(tool, str) else ""
